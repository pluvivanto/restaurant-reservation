...
class ReservationControllerTest {
    ...
    void updateStatus_ReturnsUpdatedDto_WhenStatusValid() throws Exception {
        ...
        String statusRequestJson =
                java.util.Objects.requireNonNull(objectMapper.writeValueAsString(statusRequest));

        // when
        mockMvc
                .perform(post("/reservations/{id}/status", reservation.getId())
                        .contentType(MediaType.APPLICATION_JSON_VALUE).content(statusRequestJson))
                // then
                .andExpect(status().isOk())
    ...
    void listReservations_ReturnsFilteredList_WhenDateProvided() throws Exception {
        ...
        when(reservationService.listReservations(reservation.getRestaurantId(), date))
                .thenReturn(List.of(reservation));

        // when
        mockMvc
                .perform(get("/restaurants/{id}/reservations", reservation.getRestaurantId()).param("date",
                        date.toString()))
                // then
                .andExpect(status().isOk()).andExpect(jsonPath("$[0].id").value(reservation.getId()));

        verify(reservationService).listReservations(reservation.getRestaurantId(), date);
    }
}
